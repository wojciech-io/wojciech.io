...
The backend is a Postgres database with a straightforward availability model: pitch inventory, date blocks, reservations. The core availability check is a simple overlapping intervals query: intentionally simple because it needs to be fast and correct, not clever.

Payment is Stripe. Not Przelewy24, not PayU: Stripe, because the developer experience is better and the international card coverage matters for tourist traffic. Polish card acceptance is complete through Stripe.

SMS confirmation uses Twilio. In testing, email confirmation had a 34% open rate within 10 minutes. SMS had a 94% open rate within 2 minutes. For a booking confirmation that includes gate code and pitch number, that difference matters operationally.

Admin is a simple CRUD interface with a visual availability calendar. The most-used views are: today's arrivals, this week's calendar, and the pricing editor. Everything else is secondary.
