...
tldr:
  ...
  - "Operator CMO is open source: nine skills, three layers, MIT, installable in Claude Code"
faq:
...
<PullQuote
  ...
  source="A commenter under the original post"
...
<KeyPoint type="takeaway" title="The failure you will not notice for a quarter">
...
<Diagram caption="Execution reads the decision before it produces anything. Miss the file, or run past the quarter's end date, and the skill stops instead of guessing.">
  <Stack direction="column" gap="sm" align="stretch">
    <Box variant="accent" label="positioning.md" sub="who we serve, who we refuse, what we may claim" />
    <Box variant="accent" label="quarter.md" sub="two or three bets, one metric, a kill date" />
  </Stack>
...
<StackLayer
  title="Operator CMO: three layers, nine skills"
  layers={[
    { name: "start", tech: "Guided first run: five steps, one question cluster at a time", badge: "setup" },
...
<CodeBlock filename="marketing/drafts/2026-08-28-month-end-close.md" lang="yaml" caption="The header is the point. An asset that cannot name the decision it came from is not publishable.">
bet: B2
...
<AnnotatedImage
  ...
  alt="A request card floating above three bet cards labelled B1, B2 and B3, with dashed lines that stop short and end in crosses instead of connecting."
  caption="A request that matches no bet is not written. It is recorded as a refusal, with the price of changing the plan spelled out next to it."
/>
...
<AnnotatedImage
  ...
  alt="Three horizontal lanes labelled A, B and C. Lane A runs free, lane B has a gate part way along, lane C has a gate at the start and a queue of work piled up behind it."
  caption="Tier A runs free. Tier B is sampled. Tier C is read every time, and that is where the queue forms if production is not capped."
/>
...
<Diagram caption="Throughput is what ships, not what gets generated. Reporting the left-hand number is how these projects get funded and then quietly cancelled.">
  <Box variant="muted" label="produced" sub="40 / week" />
  <Arrow direction="right" label="gate" />
  <Box variant="accent" label="shipped" sub="10 / week" />
  <Arrow direction="right" dashed label="overflow" />
  <Box variant="outline" label="backlog" sub="30 and growing" />
</Diagram>
...
<ToolCard
  title="What this responds to, and what it ships"
  cols={2}
  items={[
    {
      name: "The original post",
      description: "Ignacio Prieto's seven-layer Claude marketing stack, and the comment thread underneath it that asked who made the plan.",
      href: "https://www.linkedin.com/posts/ignacioprietom_i-could-not-afford-a-cmo-so-i-built-one-share-7497741014326009857-7mlJ/",
      meta: "linkedin.com",
      icon: "chat",
    },
    {
      name: "operator-cmo",
      description: "The plugin this article argues for. Nine skills, the decision guard, the ship gate, templates and a worked example.",
...
<Terminal
  ...
  commands={[
    { cmd: "/operator-cmo:start", output: "Guided first run. Five steps, one question cluster at a time." },
    { cmd: "/operator-cmo:positioning", output: "The decision file everything else reads." },
    { cmd: "/operator-cmo:quarter", output: "Two or three bets, each with a kill condition." },
    { cmd: "/operator-cmo:ship-gate", output: "What may go out, and how fast." },
    { cmd: "/cmo", output: "State of the system, any time." },
  ]}
/>
