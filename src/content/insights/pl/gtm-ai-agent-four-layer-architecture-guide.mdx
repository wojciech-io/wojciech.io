...
<Diagram direction="column" caption="Każda warstwa stoi na poprzedniej. Tożsamość ładuje się pierwsza i wiąże resztę. Skille działają na końcu, bo zakładają, że trzy pozostałe już są na miejscu.">
  <Box variant="accent" label="Warstwa 1: CLAUDE.md" sub="Tożsamość operatora, zasady działania" />
  <Arrow label="ładuje się pierwsza, wiąże resztę" />
  <Box label="Warstwa 2: Narzędzia MCP" sub="Web search, zapis CRM, draft emaila" />
  <Arrow label="działa na realnych systemach" />
  <Box label="Warstwa 3: /memory" sub="ICP, historia kont, obiekcje" />
  <Arrow label="trwa między sesjami" />
  <Box variant="outline" label="Warstwa 4: Skille" sub="research · enrich · draft · score · update" />
</Diagram>
...
Narzędzia [MCP (Model Context Protocol)](https://modelcontextprotocol.io/) dają agentowi zdolność działania na zewnętrznych systemach, nie tylko generowania tekstu.
...
To jest ta warstwa, dzięki której praca się kumuluje. Research z wtorku nadal tam leży w czwartek. Odpowiedź na obiekcję, którą zatwierdziłem miesiąc temu, pojawia się w każdej kolejnej sesji, bez wklejania jej z powrotem.

Nikt nie briefuje agenta od nowa. On czyta pliki.

## Warstwa 4: Skille operatora, czyli reużywalne programy promptowe

Skill to program promptowy z jednym zadaniem. Starter pack wiezie pięć:
...
Każdy deklaruje swoje wejścia, kształt wyjścia i zasady, a potem wołasz go po nazwie: `/research Acme Corp`. Agent czyta definicję, wykonuje ją i oddaje coś o przewidywalnym kształcie, i o to właśnie chodzi. Odpowiedź, którą musisz ręcznie przeformatować, to odpowiedź, którą napisałeś sam.

## Dlaczego warstwy mają znaczenie

Większość setupów GTM opartych na promptach rozpada się z jednego powodu. Są bezstanowe, więc każda rozmowa zaczyna się od zera, a operator zużywa pierwsze dziesięć minut na odbudowanie kontekstu, który tłumaczył tydzień wcześniej. Cztery warstwy to tyle, ile było trzeba, żeby przestać to robić:
...
<KeyTakeaway title="Pomiń warstwę i oto co się psuje">
Wytnij tożsamość, a agent dopytuje o kontekst co sesję. Wytnij narzędzia, a masz pisarza, nie operatora. Wytnij pamięć, a research z wtorku znika do czwartku. Wytnij skille, a jakość zależy od tego, jak sformułowałeś prośbę tego ranka. Te cztery warstwy to nie funkcje. To cztery sposoby, w jakie agent GTM przestaje być chatbotem.
</KeyTakeaway>

---

**Powiązane:** [Narzędzia GTM: framework decyzji build vs buy dla operatorów](/pl/insights/gtm-tools-build-vs-buy-decision-framework)
