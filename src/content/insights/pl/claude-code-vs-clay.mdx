---
title: "Pakiet startowy dla agentów Claude Code GTM"
description: "Jak stworzyłem czterowarstwowy system AI, który przeprowadza badania zewnętrzne, tworzy sekwencje i wzbogaca rekordy w bazie danych CRM – bez użycia ani jednego zewnętrznego narzędzia do wzbogacania danych."
...
Większość treści na stronie GTM AI można podzielić na dwie kategorie: porady dotyczące poleceń dla ChatGPT albo porównania narzędzi za 500 dolarów miesięcznie, z których tak naprawdę nikt nie może skorzystać. To nie jest ani jedno, ani drugie.

Oto opis działającego systemu, który stworzyłem i obsługuję: czterowarstwowy stos agentów Claude Code, który zajmuje się wyszukiwaniem zewnętrznym, tworzeniem sekwencji, wzbogacaniem danych (CRM) oraz oceną wyników (ICP). Bez Clay. Bez punktów za wzbogacanie danych w Apollo. Bez przepływów w n8n, które psują się co trzy tygodnie.

Wystarczy Claude Code, kilka narzędzi z MCP, dobrze zorganizowany plik CLAUDE.md oraz pięć połączonych ze sobą umiejętności operatorów.

---

## Dlaczego Claude Code zamiast Clay

Clay Sprawdza się znakomicie w tym, do czego służy: oferuje uporządkowane procesy wzbogacania danych z intuicyjnym interfejsem i modelem kredytowym dla każdego wiersza. Korzystałem z tego narzędzia. Jest ono naprawdę przydatne dla zespołów, które potrzebują wzbogacania danych na dużą skalę, ale nie dysponują zasobami inżynieryjnymi.

Ma jednak pewne ograniczenia, które mają znaczenie dla mojego sposobu pracy:

- Kupujesz punkty wzbogacające dane, a nie wiedzę. Usługa Clay przekazuje dane za pośrednictwem zewnętrznych dostawców, takich jak Clearbit, Apollo i narzędzia do zbierania danych z LinkedIn, a opłaty naliczane są za każdy wiersz. Warstwa analityczna LLM jest uboga.
...
Claude Code, a konkretnie pętla agenta za pośrednictwem SDK z wbudowanymi narzędziami MCP, pozwala mi stworzyć coś wyjątkowego: system, który **analizuje kontekst, ocenia trafność, generuje treść i aktualizuje rekordy** – wszystko w jednym cyklu. To inteligencja jest produktem, a nie infrastruktura.
...
**Wyszukiwanie w Internecie** (za pośrednictwem przeglądarki Brave MCP lub Exa MCP): w celu uzyskania informacji o firmach w czasie rzeczywistym, wiadomości dotyczących finansowania, ofert pracy oraz najnowszych treści. Funkcja ta zastępuje etap „wzbogacania danych metodą kaskadową” w Clay.

**Automatyzacja przeglądarki** (za pomocą biblioteki Puppeteer MCP): w przypadku stron blokujących roboty indeksujące. Strony firmowe na LinkedIn, niektóre strony z cennikami, dane z serwisu Glassdoor dotyczące wielkości zespołów i tempa wydatkowania środków.

**Pamięć / system plików** (za pośrednictwem systemu plików Claude MCP): do odczytu i zapisu plików klienta: plik eksportu CSV z serwisu CRM, szablony sekwencji oraz dziennik aktywności dotyczący skontaktowanych kont.

**Niestandardowy moduł oceny MCP** (stworzony w ciągu weekendu przy użyciu zestawu SDK MCP): pobiera ustrukturyzowany obiekt JSON (dane firmy + kryteria ICP) i zwraca wynik dopasowania wraz z uzasadnieniem. Było to najtrudniejsze do zrealizowania, ale też najbardziej wartościowe rozwiązanie: zapewnia agentowi spójną skalę oceny we wszystkich przebiegach.
...
**`/draft [firma] [wariant-sekwencji]`**: Odczytuje plik z wynikami badań, ocenę oraz szablon sekwencji, a następnie tworzy spersonalizowaną pierwszą wiadomość e-mail oraz dwie wiadomości uzupełniające. Nigdy nie korzysta z pól scalania. Zawsze tworzy treść od podstaw w oparciu o rzeczywiste wyniki badań.
...
Porównaj z Clay: 800–2400 USD miesięcznie za plan zapewniający znaczące kredyty na wzbogacanie treści oraz LLM wierszy. Biorąc pod uwagę mój wolumen i sposób wykorzystania, stos agentów kosztuje około 5–10% tej kwoty, a jakość wyników na poziomie wnioskowania i tworzenia treści jest lepsza.
...
**Monitorowanie sygnałów**: lekki skrypt cron, który co tydzień uruchamia polecenie `/score` w odniesieniu do listy obserwowanych kont i sygnalizuje czynniki sprzyjające ponownemu zaangażowaniu (nowe finansowanie, zmiana kierownictwa, ogłoszenia o pracy na odpowiednich stanowiskach).

Oba projekty są w trakcie realizacji. Opiszę je tutaj, gdy osiągną stabilną wersję.

---

Jeśli tworzysz coś podobnego lub chcesz zobaczyć, jak działa ten pakiet startowy, [umów się na rozmowę](https://cal.com/wojciech-luszczynski). Chętnie omówię z Tobą jego strukturę.
