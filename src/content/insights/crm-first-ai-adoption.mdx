...
A GTM team gets access to an AI tool. They start using it for the highest-visibility, lowest-friction task: writing. Email subject lines. LinkedIn posts. Sales one-pagers. I wrote about this pattern in more detail in [the GTM build vs buy framework](/insights/gtm-tools-build-vs-buy-decision-framework/). The output is better than what they were producing manually, and it's faster, so adoption feels successful.
...
- **Enrichment on new contacts.** When a new contact enters the CRM, pull firmographic data (via [Apollo](https://www.apollo.io/) or [Clay](https://www.clay.com/)) and run an ICP score. Don't wait for a human to do it manually.
...
Those are operator-level questions. The AI surfaces the data. The operator reads it and decides. That's the right division of labour. I built an example of this kind of decision surface in [the Google Ads dashboard guide](/insights/google-ads-ai-management-dashboard-guide/).
...
I've seen this pattern enough times that I now diagnose it in the first hour of any GTM engagement. Before we talk about what AI tools to add, I check three things:

<Tabs labels={['ICP definition', 'CRM field completion', 'Pipeline stages']}>
  <Fragment slot="0">Is the ICP specific enough to score a company against it and get a consistent answer? "Mid-market B2B SaaS" is not an ICP. Headcount range, industry, tech stack signals, behavioral indicators: that is an ICP.</Fragment>
  <Fragment slot="1">What percentage of contacts have company associations? Are job titles normalized? Are segment fields filled? An empty field is not a data problem waiting to be fixed. It is a broken signal your AI tools will ignore or hallucinate around.</Fragment>
  <Fragment slot="2">Do stages reflect buyer behavior or seller hope? "Proposal Sent" is a seller action. "Champion confirmed, procurement engaged" is a buyer signal. If stages are the first type, the pipeline numbers are aspirational, not diagnostic.</Fragment>
</Tabs>
...
## The client asked for a sequence tool. We spent two weeks on the CRM.
