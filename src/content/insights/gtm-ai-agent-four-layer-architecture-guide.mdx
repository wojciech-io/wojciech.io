...
[CLAUDE.md](https://docs.anthropic.com/en/docs/claude-code/memory) is the first file [Claude Code](https://docs.anthropic.com/en/docs/claude-code) reads when it starts a session. It defines who the agent is, not what it can do. (Grab my [GTM starter template](/bites/) to skip the blank-page problem.)
...
<DoDont
  doLabel="System (4 layers)"
  dontLabel="Chatbot (no layers)"
  do={[
    "Identity persists across sessions (CLAUDE.md)",
    "Tools connect to real systems (MCP)",
    "Context accumulates in memory files",
    "Skills produce consistent, structured output",
  ]}
  dont={[
    "Re-explain context every session",
    "Copy-paste between browser tabs",
    "Lose research when the chat closes",
    "Write prompts from scratch each time",
  ]}
/>

The cost per full outbound run (research, enrich, draft, log) is $2-8 in [Claude](https://claude.ai) API costs. An equivalent commercial stack ([Clay](https://clay.com) credits + [Apollo](https://apollo.io) seats + [Outreach](https://outreach.io)) runs $200-500/month for the same output volume.

The gap is not capability. It is the operating model.
