...
I do not need a full application framework for a public portfolio and writing system. I need strong file-based routing, components, content collections, MDX, and output that [Cloudflare Pages](https://pages.cloudflare.com/) can serve cheaply and reliably.
...
<Tabs labels={['Current shape', 'When to upgrade']}>
  <Fragment slot="0">One owner. A small number of public pages. English-only insights. Fast agent-assisted patches. Source-controlled changes. No editorial team. For that shape, the debt is cheaper than the abstraction.</Fragment>
  <Fragment slot="1">If this becomes a high-volume publication: real locale content model. If multiple editors need publishing rights: a CMS. If article count passes 50: probably a search index. Until then, those systems would be ceremony that slows down shipping.</Fragment>
</Tabs>
