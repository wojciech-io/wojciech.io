...
**Context quality is the multiplier.** The `CLAUDE.md` is the investment. The operator who briefs it well — specific ICP signals, real objection patterns, defined tone rules — gets output that compounds. Run five on a well-structured engagement is measurably better than run one. That compounding doesn't happen with row-processing enrichment tools. They don't accumulate operator context.

**Information-dense inputs produce information-dense output.** The richer the ICP definition, the cleaner the CRM export, the more specific the brief: the better the agent output. The principle is the same as everywhere: the quality of the reasoning is bounded by the quality of the inputs.

**The judgment layer stays with the operator.** Claude Code executes. The scoring logic, the sequence strategy, the positioning decisions are mine. The speed improvement is real and measurable. The strategic layer is not delegated.
