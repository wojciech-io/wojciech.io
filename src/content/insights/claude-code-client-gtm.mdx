...
**Context depth.** The `CLAUDE.md` is the leverage point. A sparse brief produces sparse output. A detailed operator context: ICP signals, objection patterns, account notes accumulated over months, produces output a new hire couldn't replicate. The upfront investment in setup is what makes every subsequent run compounding rather than just fast.

**Task density.** The richer the source material, the sharper the output. A well-documented CRM yields better segmentation. A detailed ICP yields better sequences. The agent amplifies quality; it doesn't manufacture it.

**Operator judgment stays at the centre.** The scoring logic, sequence strategy, and positioning decisions come from me. Claude Code executes the research and drafts. The commercial judgment is on the operator, which is precisely why it works. You don't want the agent making positioning calls. You want it executing the ones you've already made, faster and more consistently than any team could.
